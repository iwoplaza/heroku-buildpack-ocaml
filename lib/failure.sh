...
fail_invalid_package_json() {
  ...
  is_invalid=$(is_invalid_json_file "${1:-}/esy.json")

  if "$is_invalid"; then
    error "Unable to parse esy.json"
    mcount 'failures.parse.esy-json'
    meta_set "failure" "invalid-esy-json"
    header "Build failed"
    failure_message
    fail
  fi
...
warn_missing_esy_json() {
  local build_dir=${1:-}
  if ! [ -e "$build_dir/esy.json" ]; then
    warning "No esy.json found"
