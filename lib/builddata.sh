...
log_initial_state() {
  meta_set "buildpack" "ocaml"
  meta_set "ocaml-package-manager" "esy"
...
log_project_info() {
  ...
  meta_set "uses-workspaces" "$(json_has_key "$build_dir/esy.json" "workspaces")"
  # What workspaces are defined? Logs as: `["packages/*","a","b"]`
  meta_set "workspaces" "$(read_json "$build_dir/esy.json" ".workspaces")"
